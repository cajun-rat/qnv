...
MainWindow::MainWindow(QWidget* parent)
  ...
  connect(m_ui->search, SIGNAL(textChanged(QString)), this,
          SLOT(updateSearch(void)));
  connect(m_ui->currentNote, SIGNAL(textChanged(void)), this,
  ...
  // enter in search box
  connect(m_ui->search, SIGNAL(returnPressed()), this,
          SLOT(searchEnterPressed()));

  connect(m_ui->actionChange_Notes_Directory, SIGNAL(triggered()), this,
...
void MainWindow::updateSearch() {
  ...
  bool foundOne = false;
  for (std::vector<NoteListWidget*>::iterator i = m_notes.begin();
       i != m_notes.end(); ++i) {
